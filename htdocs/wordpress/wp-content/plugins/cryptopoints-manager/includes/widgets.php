<?php
/**
 * Cryptopoints Manager widgets
 */

/** Points Manager Points Widget declaration */
class cpm_pointsWidget extends WP_Widget {

	// constructor
	function cpm_pointsWidget() {
		parent::WP_Widget( 'pm_pointsWidget', 'Points Manager', array( 'description' => 'Display the points of the current logged in user.' ) );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( !( !is_user_logged_in() && $instance['text_alt'] == '' ) ) {
			...
			if ( is_user_logged_in() ) {
				...
				$string = str_replace( '%points%', '<span class="pm_points_display">' . cpm_displayPoints( 0, 1, 1 ) . '</span>', $instance['text'] );
			} else {
				$string = $instance['text_alt'];
			}

			//start output
			do_action( 'pm_pointsWidget_before' );
			if ( $instance['html'] == '' ) {
				?>
				<ul>
					<li><?php echo $string; ?></li>
					<?php do_action( 'cpm_pointsWidget' ); ?>
				</ul>
			<?php
			} else {
				echo str_replace( '%text%', $string, $instance['html'] );
			}
			do_action( 'cpm_pointsWidget_after' );
			echo $after_widget;
		}
	}
...
/** Points Manager Top Users Widget */
class cpm_topUsersWidget extends WP_Widget {

	// constructor
	function cpm_topUsersWidget() {
		parent::WP_Widget( 'pm_topUsersWidget', 'Cryptopoints Manager Top Users', array( 'description' => 'Use this widget to showcase the users with the most points.' ) );
	}
	...
	function widget( $args, $instance ) {
		...
		$top = cpm_getAllPoints( $instance['num'], get_option( 'pm_topfilter' ) );
		do_action( 'cpm_topUsersWidget_before' );
		echo apply_filters( 'pm_topUsersWidget_before', '<ul>' );
		$line = apply_filters( 'pm_topUsersWidget_line', '<li class="pm_topUsersWidget top_%place%" style="%style%">%string%</li>' );
		...
		foreach ( $top as $x => $y ) {
			...
			$string = apply_filters( 'cpm_displayUserInfo', $string, $y, $x + 1 );
			echo $string;
		}
		echo apply_filters( 'cpm_topUsersWidget_after', '</ul>' );
		do_action( 'cpm_topUsersWidget_after' );
		echo $after_widget;
	}
...
add_action( 'widgets_init', 'cpm_widgets' );

function cpm_widgets() {
	register_widget( "cpm_pointsWidget" );
	register_widget( "cpm_topUsersWidget" );
}
